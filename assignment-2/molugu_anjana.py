...
if __name__ == "__main__":
    ...
    wikiPages = sc.textFile(sys.argv[1]) #("WikipediaPagesOneDocPerLine1000LinesSmall.txt.bz2") 
    wikiCategoryLinks=sc.textFile(sys.argv[2]) # ("wiki-categorylinks-small.csv.bz2")
   ...
   #print(topWords.take(2))
    ...
    # i^th word in the corpus  Anjana check
    ...
    task2prediction1result.coalesce(1).saveAsTextFile(sys.argv[3]) #(task2prediction1)
    ...
    task2prediction2result.coalesce(1).saveAsTextFile(sys.argv[4]) #("task2prediction2")
    ...
    task2prediction3result.coalesce(1).saveAsTextFile(sys.argv[5]) #("task2prediction3")
